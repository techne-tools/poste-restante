...
import { doorName, STATE_LABEL, daysUntil, clauseClass } from "../utils/bookUtils";
...
interface Props {
  ...
  name?: string;
  /** The resident's handle — the book hides each resident's power to
   *  vouch for their own proposal (rule 10: you may develop, you may not
   *  support yourself). */
  address?: string;
}

export default function Book({ onError, initialClause, name, address }: Props) {
  ...
  /** The thread being supported — the inline draft that stands WITH the
   *  norm (a support is a chance to add context, thoughts, and value
   *  statements — never a bare button, rule 9). */
  ...
  /** Open the support draft — standing with the norm, in words. A support
   *  is an opportunity to add context, thoughts, and value statements; it
   *  is never a bare button (rule 9). */
  const startSupport = useCallback((thread: string) => {
    setThreadLetters(null);
    setOpenThread(thread);
    setSupportDraft("");
    setSupporting(thread);
  }, []);

  const support = useCallback(async () => {
    if (!supporting || !supportDraft.trim()) return;
    setActing(supporting);
    try {
      await house.actOnBook({ role: "support", continues: supporting, text: supportDraft });
      setSupporting(null);
      setSupportDraft("");
      await refresh();
    } catch (err) {
      onError(err instanceof Error ? err.message : "the book could not hold this support");
    } finally {
      setActing(null);
    }
  }, [supporting, supportDraft, refresh, onError]);
          ...
          {standing.map((c) => (
            <article key={c.thread} className={clauseClass(c.state)}>
              ...
              <div className="clause-meta">
                ...
                {c.vouches > 0 && (
                  <span>
                    {c.vouches} support{c.vouches === 1 ? "" : "s"}
                    {c.supportsTowardStanding > 0 && c.supportsTowardStanding < c.vouches
                      ? " — " + c.supportsTowardStanding + " toward standing"
                      : ""}
                  </span>
                )}
              ...
              <div className="clause-actions">
                <button
                  ...
                  disabled={acting === c.thread || c.proposedBy === address}
                  title={
                    c.proposedBy === address
                      ? "you offered this — support is another voice; develop instead to move it"
                      : undefined
                  }
                  onClick={() => startSupport(c.thread)}
                >
                  support
                </button>
                <button
              ...
              {supporting === c.thread && (
                <ClauseDevelop
                  ...
                  onDevelop={support}
                  onCancel={() => setSupporting(null)}
                  actLabel="Stand with it"
                  heading="Standing with the norm — what do you see in it?"
                />
              )}
              {developing === c.thread && (
                <ClauseDevelop
          ...
          {proposed.map((c) => (
            <article key={c.thread} className={clauseClass(c.state)}>
              ...
              <WhyNote summary="how the household decides">
                ...
                stands with the norm, in words; stop is a safe word that holds
                the clause contested; develop rewrites it and starts the
                clock again. Nothing stands until it has been held, quiet,
                for the settling days AND carried by a support — the commons
                is supported, never merely timed, and no one supports their
                own offer (the offerer develops). The commons is derived,
                never declared (no one is master; everyone can be inscribed).
              </WhyNote>
              {supporting === c.thread && (
                <ClauseDevelop
                  draft={supportDraft}
