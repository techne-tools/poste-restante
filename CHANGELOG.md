...
- **Participation is derived.** The `thread_participation` cache holds the current state per (thread, address): 'in' or 'out', set by the latest leave/join letter (by received_at — the upsert guard refuses out-of-order writes). Addresses with no leave/join letter are 'in' by default — the historical edges stand. Wiping and re-deriving from the letters yields the same rows (the rememberer's cache, like `clauses`). The pipeline fires the hook on the single write path. (`server/src/pipeline/pipeline.ts`, `server/src/index.ts`)
- **Visibility prunes itself.** `isVisibleTo` and `visibleToSql` gain the derived-participation limb: a letter is visible to an address iff the address is a participant AND currently 'in' the thread. The whisper's `VISIBLE_TO` and the gap engine's detection queries gain the same limb — a leaver stops being offered the thread, and gaps are not even created for it (convergent by construction). A leaver gets 404, never 403 — absence is silence. (`server/src/auth/visibility.ts`, `server/src/whisper/service.ts`)
- **The book is exempt.** Clause threads are commons by right — you cannot leave the household's knowing of itself. Leave/join on clause threads is refused (400). (`server/src/server.ts`, `server/src/mcp/server.ts`)
- **The reference client's left state.** "You have left this correspondence." — calm, no red, with the rejoin action. The archive keeps the history; the house has stopped whispering about it. (`client/src/ThreadView.tsx`, `client/src/api.ts`, `client/src/KindTag.tsx`)
- **Symmetric by construction.** The same move, available to everyone, protects everyone: you leave to stop seeing someone; they leave to stop seeing you. The mechanism is identical in both directions.
- **Tests** — 7 unit tests for the visibility limb (participant/leaver/public/default), 9 integration tests proving the full arc against live infra: leave → the leaver's edges dissolve (404) → the whisper stops offering → gaps are not created → rejoin → the historical edges stand again; plus the negatives (guest 401, non-participant 404, book exempt, other participants unaffected). 172/172 server, 43/43 client; typecheck and build clean.
