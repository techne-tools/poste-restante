...
- **Keys are client-held; the house stores the public halves only.** `POST /v1/addresses/:id/keys` registers `{ageRecipient, ed25519Public, recoveryAgeRecipient}` (SPEC §15) — self-only, idempotent upsert. The private halves never leave the client (they live in the client's localStorage keystore; the house never sees, stores, or logs them). Public keys are public: the address book carries `ageRecipient`/`ed25519Public` on every address row (null for a keyless legacy address) so correspondents can seal and verify. The ed25519 fingerprint becomes the address's `identity_id` — the identity IS the key once keys exist.
